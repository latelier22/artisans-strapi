...
async function fetchPages(pageSlug = null) {
  ...
  // let url = `/api/notes?populate=*&filters[channels][name][$eq]=${channel}`;
  let url = `/api/notes?populate=*`;
  ...
  const response = await myFetchMobile(url, "GET", null, "notes");
  ...
  if (pageSlug) {
    return notes.length > 0 ? pages[0] : null;
  }
