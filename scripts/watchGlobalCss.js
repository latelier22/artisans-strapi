...
async function uploadGlobalCss(styleId) {
  ...
  try {
    ...
    if (!response || response.error) {
      throw new Error(`Failed to upload CSS to Strapi: ${response ? response.error : 'Unknown error'}`);
    }
...
// Initialize watcher.
const watcher = chokidar.watch(path.resolve(__dirname, '../app/globals-strapi.css'), {
  persistent: true
});
