...
const useSiteStore = create(set => ({
  ...
  fetchAndSetSite: async () => {
    try {
      ...
      const logoThumbnailUrl = data.attributes.logo?.data?.attributes?.formats?.thumbnail?.url ? `${baseUrl}${data.attributes.logo.data.attributes.formats.thumbnail.url}` : null;
      const footerImageUrl = data.attributes.footerImage?.data?.attributes?.url ? `${baseUrl}${data.attributes.footerImage.data.attributes.url}` : null;
      const site = {
        id: data.id,
        ...data.attributes,
